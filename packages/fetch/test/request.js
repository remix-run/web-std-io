...
describe("Request", () => {
		...
		forbiddenMethods.forEach((method) => {
			try {
				...
				expect(e.message).to.equal(
					`Failed to construct 'Request': '${method.toLowerCase()}' HTTP method is unsupported.`
				);
			}
			try {
				...
				expect(e instanceof TypeError).to.equal(true);
				expect(e.message).to.equal(
					`Failed to construct 'Request': '${method.toUpperCase()}' HTTP method is unsupported.`
				);
			}
		});
	});
	...
	it("should respect custom credentials value", () => {
		expect(new Request(base, { credentials: "omit" })).to.have.property(
			"credentials",
			"omit"
		);
		expect(new Request(base, { credentials: "include" })).to.have.property(
			"credentials",
			"include"
		);
	});
	...
	it("should throw error with GET/HEAD requests with body", () => {
		...
		expect(() => new Request(base, { body: "", method: "HEAD" })).to.throw(
			TypeError
		);
		expect(() => new Request(base, { body: "a", method: "HEAD" })).to.throw(
			TypeError
		);
		expect(() => new Request(base, { body: "a", method: "get" })).to.throw(
			TypeError
		);
		expect(() => new Request(base, { body: "a", method: "head" })).to.throw(
			TypeError
