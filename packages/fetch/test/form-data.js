...
describe("FormData", () => {
	it("should return a length for empty form-data", () => {
		...
		expect(getFormDataLength(form, boundary)).to.be.equal(
			Buffer.byteLength(getFooter(boundary))
		);
	});
	...
	it("should create a body from empty form-data", async () => {
		...
		expect(String(await read(formDataIterator(form, boundary)))).to.be.equal(
			getFooter(boundary)
		);
	});
	...
	it("should set default content-type", async () => {
		...
		expect(String(await read(formDataIterator(form, boundary)))).to.contain(
	...
	it("should create a body with a FormData Blob field", async () => {
		...
		const expected =
			`--${boundary}${carriage}` +
			'Content-Disposition: form-data; name="blob"; ' +
			`filename="blob"${carriage}Content-Type: text/plain${carriage.repeat(
				2
			)}` +
			"Hello, World!" +
			`${carriage}${getFooter(boundary)}`;
